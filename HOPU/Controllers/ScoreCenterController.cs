...
namespace HOPU.Controllers
{
    ...
    public class ScoreCenterController : Controller
    {
        ...
        public JsonResult AdminGetSelfScore(int limit, int offset, string keyword, string sortOrder, string sortName)
        {
            ...
            HopuDBDataContext db = new HopuDBDataContext();
            string sql = AdminSelfGetSql(keyword, sortName, sortOrder);
            //数据请求
        ...
        /// <param name="sortOrder">排序的方式</param>
        /// <returns>返回要查询的sql语句</returns>
        public static string AdminSelfGetSql(string keyword, string sortName, string sortOrder)
        {
            string sql = @"SELECT * FROM SelfTestScore";
            ...
            if (keyword != "")
            {
                ...
                {
                    sql += i == 0 ? " WHERE " : " AND ";
                    sql += @"CONCAT(Id,StId,RealUserName,UserName,EndTime,Score) "
                    + "LIKE '%" + keywordList[i] + "%'" + "";
                }
            }
            //排序
        ...
        /// <param name="sortOrder">排序的方式</param>
        /// <returns>返回要查询的sql语句</returns>
        public static string GetSelfSql(string UserName, string keyword, string sortName, string sortOrder)
        {
            string sql = @"SELECT * FROM SelfTestScore";
        ...
        public ActionResult Score_S(int? Id)
        {
            if (IsAdmin())
            {
                var pageNumber = Id ?? 1;
                var utList = UnifiedTestListViewModel.GetStId();
                ViewBag.UtId = pageNumber;
                return View(utList);
            }
            return View();

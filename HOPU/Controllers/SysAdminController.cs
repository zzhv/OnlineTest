    ...
    public class SysAdminController : Controller
    {
        ...
        public async Task<ActionResult> AdminLogin(LoginViewModel model, string returnUrl)
        {
            ...
            if (string.IsNullOrEmpty(returnUrl))
                returnUrl = "~/SysHome/Index";
            // 这不会计入到为执行帐户锁定而统计的登录失败次数中
            // 若要在多次输入错误密码的情况下触发帐户锁定，改为 shouldLockout: true
        ...
        public async Task<ActionResult> AdminRegister(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.UserName, RealUserName = model.RealUserName };
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("Home", "SysHome");
                }
                AddErrors(result);
            }

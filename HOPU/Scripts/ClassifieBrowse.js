...
$(this).keydown(function (event) { //监听键盘按下时的事件
    ...
    if ($("#thisTopicId").val() <= $("#maxTopicId").val() && $("#thisTopicId").val() >= $("#minTopicId").val()) {
...
$(function () {
    $("#tiaozhuan").on("keyup", function () {
        ...
        //if ($("tiaozhuan").val() * 1 < $("#minTopicId").val() * 1 && $("tiaozhuan").val() * 1 > $("#maxTopicId").val() * 1) {
        //    $("#tiaozhuan").val($("#minTopicId").val())
        //}
        $("input[name='answerIteam']").removeAttr("disabled");
        $("#answerIsRight").css("display", "none");
